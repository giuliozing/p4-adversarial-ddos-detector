...
from tensorflow.keras.layers import Input, Dense, Concatenate, Add, BatchNormalization , Dropout, Lambda
...
# Carica la LUT di randomizzazione
LUT_PATH = os.path.join('LUT', 'LUT_Randomness.csv')
lut_df = pd.read_csv(LUT_PATH, header=None)
lut_array = lut_df.values[:, 1:].astype(np.float32)  # Esclude la prima colonna (indice)

def apply_randomization(features):
    """
    Applica randomizzazione alle features basata su hash CRC.
    features: tensor di shape (batch_size, num_features)
    """
    @tf.custom_gradient
    def randomize_with_gradient(x):
        def randomize_fn(x_input):
            # Converti il tensor in numpy per calcolare CRC
            x_np = x_input.numpy()
            ...
            for i in range(batch_size):
                # Calcola CRC32 delle features e prendi modulo 256
                features_bytes = x_np[i].tobytes()
                crc_hash = zlib.crc32(features_bytes) % 256
                
                # Recupera i valori dalla LUT
                lut_values = lut_array[crc_hash, :num_features]
                
                # Aggiungi i valori alle features
                result[i] = x_np[i] + lut_values
            
            return result.astype(np.float32)
        
        result = tf.py_function(randomize_fn, [x], tf.float32)
        result.set_shape(x.shape)
        
        # Definisce il gradiente personalizzato (passa il gradiente invariato)
        def grad(dy):
            return dy
        
        return result, grad
    
    return randomize_with_gradient(features)
...
def nn(bitwidth, LUT,class_number):
    ...
    if LUT == 1:
        ...
        # Applica randomizzazione basata su CRC
        combined_input = Concatenate()([model1_in_1, model1_in_2])
        randomized_input = Lambda(lambda x: apply_randomization(x))(combined_input)
        randomized_1 = Lambda(lambda x: tf.expand_dims(x[:, 0], axis=-1))(randomized_input)
        randomized_2 = Lambda(lambda x: tf.expand_dims(x[:, 1], axis=-1))(randomized_input)
        
        model1_in_1_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model1_in_1_quant')(randomized_1)
        model1_in_2_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model1_in_2_qaunti')(randomized_2)
    ...
    if LUT == 3:
        ...
        # Applica randomizzazione basata su CRC
        combined_input_1 = Concatenate()([model1_in_1, model1_in_2])
        randomized_input_1 = Lambda(lambda x: apply_randomization(x))(combined_input_1)
        randomized_1_1 = Lambda(lambda x: tf.expand_dims(x[:, 0], axis=-1))(randomized_input_1)
        randomized_1_2 = Lambda(lambda x: tf.expand_dims(x[:, 1], axis=-1))(randomized_input_1)
        
        model1_in_1_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model1_in_1_quant')(randomized_1_1)
        model1_in_2_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model1_in_2_qaunti')(randomized_1_2)
        model_1_concatenated = Concatenate()([model1_in_1_quant, model1_in_2_quant])
        ...
        # Applica randomizzazione basata su CRC
        combined_input_2 = Concatenate()([model2_in_1, model2_in_2])
        randomized_input_2 = Lambda(lambda x: apply_randomization(x))(combined_input_2)
        randomized_2_1 = Lambda(lambda x: tf.expand_dims(x[:, 0], axis=-1))(randomized_input_2)
        randomized_2_2 = Lambda(lambda x: tf.expand_dims(x[:, 1], axis=-1))(randomized_input_2)
        
        model2_in_1_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model2_in_1_quant')(randomized_2_1)
        model2_in_2_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model2_in_2_quanti')(randomized_2_2)
    ...
    if LUT == 5:
        ...
        # Applica randomizzazione basata su CRC
        combined_input_3 = Concatenate()([model3_in_1, model3_in_2])
        randomized_input_3 = Lambda(lambda x: apply_randomization(x))(combined_input_3)
        randomized_3_1 = Lambda(lambda x: tf.expand_dims(x[:, 0], axis=-1))(randomized_input_3)
        randomized_3_2 = Lambda(lambda x: tf.expand_dims(x[:, 1], axis=-1))(randomized_input_3)
        
        model3_in_1_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model3_in_1_quant')(randomized_3_1)
        model3_in_2_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model3_in_2_quanti')(randomized_3_2)
        model_3_concatenated = Concatenate()([model3_in_1_quant, model3_in_2_quant])
    ...
    if LUT == 7:
        ...
        # Applica randomizzazione basata su CRC
        combined_input_4 = Concatenate()([model4_in_1, model4_in_2])
        randomized_input_4 = Lambda(lambda x: apply_randomization(x))(combined_input_4)
        randomized_4_1 = Lambda(lambda x: tf.expand_dims(x[:, 0], axis=-1))(randomized_input_4)
        randomized_4_2 = Lambda(lambda x: tf.expand_dims(x[:, 1], axis=-1))(randomized_input_4)
        
        model4_in_1_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model4_in_1_quant')(randomized_4_1)
        model4_in_2_quant = lq.quantizers.DoReFa(k_bit=bitwidth, mode="activations",name='model4_in_2_quanti')(randomized_4_2)
